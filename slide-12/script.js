/* Slide 06 : Analyse et spécification des besoins */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const actorsLabel = document.querySelector('.actors__label');
    const actorCards = document.querySelectorAll('.actor__card');
    const nfr        = document.querySelector('.nfr');
    const nfrItems   = document.querySelectorAll('.nfr__item');
    ...
    // 3. Actors label
    setTransition(actorsLabel,
        'opacity 0.6s ease, transform 0.6s ease', 800);

    // 4. Actor cards stagger
    actorCards.forEach((card, i) => {
        setTransition(card,
            'opacity 0.7s cubic-bezier(0.22,1,0.36,1), transform 0.7s cubic-bezier(0.22,1,0.36,1)', 1000 + i * 120);
    });

    // 5. NFR section
    setTransition(nfr,
        'opacity 0.7s ease, transform 0.7s ease', 1600);

    // 6. NFR items stagger
    nfrItems.forEach((item, i) => {
        setTransition(item,
            'opacity 0.35s cubic-bezier(0.34,1.56,0.64,1), transform 0.35s cubic-bezier(0.34,1.56,0.64,1)', 1800 + i * 60);
    });
